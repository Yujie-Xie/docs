...
## Error: <code>java.sql.BatchUpdateException:statement count 5001 exceeds the transaction limitation</code> while using Sqoop to write data into TiDB in batches {#error-code-java-sql-batchupdateexception-statement-count-5001-exceeds-the-transaction-limitation-code-while-using-sqoop-to-write-data-into-tidb-in-batches}
...
## Does <code>SHOW PROCESSLIST</code> display the system process ID? {#does-code-show-processlist-code-display-the-system-process-id}
...
### How long does it take to perform various DDL operations? {#how-long-does-it-take-to-perform-various-ddl-operations}
...
### Possible reasons why DDL execution is slow {#possible-reasons-why-ddl-execution-is-slow}
...
### What triggers the <code>Information schema is changed</code> error? {#what-triggers-the-code-information-schema-is-changed-code-error}
...
### Why is DDL execution blocked? {#why-is-ddl-execution-blocked}
...
### Identify the cause of stuck DDL execution {#identify-the-cause-of-stuck-ddl-execution}
...
## Collation used in JDBC connections {#collation-used-in-jdbc-connections}
...
### What collation is used in a JDBC connection when <code>connectionCollation</code> is not configured in the JDBC URL? {#what-collation-is-used-in-a-jdbc-connection-when-code-connectioncollation-code-is-not-configured-in-the-jdbc-url}
...
### How to handle collation changes after upgrading TiDB? {#how-to-handle-collation-changes-after-upgrading-tidb}
...
### What are the differences between the <code>utf8mb4_bin</code> and <code>utf8mb4_0900_ai_ci</code> collations? {#what-are-the-differences-between-the-code-utf8mb4-bin-code-and-code-utf8mb4-0900-ai-ci-code-collations}
...
### What is the ID rule when a query plan is presented as a tree? What is the execution order for this tree? {#what-is-the-id-rule-when-a-query-plan-is-presented-as-a-tree-what-is-the-execution-order-for-this-tree}
