...
# Set Up Self-Hosted Kafka Private Link Service in AWS {#set-up-self-hosted-kafka-private-link-service-in-aws}
...
| AZ IDs                              | <li>`usw2-az1` </li><li>`usw2-az2` </li><li> `usw2-az3`</li>                                                                                                                                                                                                                                                                                                                                | Align AZ IDs to AZ names in your AWS account.<br/>Example: <ul><li> `usw2-az1` => `us-west-2a` </li><li> `usw2-az2` => `us-west-2c` </li><li>`usw2-az3` => `us-west-2b`</li></ul>                                                                                                                                                                                                                                                                             |
...
## Step 1. Set up a Kafka cluster {#step-1-set-up-a-kafka-cluster}
...
### Deploy a new Kafka cluster {#deploy-a-new-kafka-cluster}

#### 1. Set up the Kafka VPC {#1-set-up-the-kafka-vpc}
...
#### 2. Set up Kafka brokers {#2-set-up-kafka-brokers}
...
### Reconfigure a running Kafka cluster {#reconfigure-a-running-kafka-cluster}
...
#### 1. Configure the EXTERNAL listener for brokers {#1-configure-the-external-listener-for-brokers}
...
#### 2. Test EXTERNAL listener settings in your internal network {#2-test-external-listener-settings-in-your-internal-network}
...
## Step 2. Expose the Kafka cluster as Private Link Service {#step-2-expose-the-kafka-cluster-as-private-link-service}

### 1. Set up the load balancer {#1-set-up-the-load-balancer}
...
### 2. Set up Private Link Service {#2-set-up-private-link-service}
...
## Step 3. Connect from TiDB Cloud {#step-3-connect-from-tidb-cloud}
...
### How to connect to the same Kafka Private Link service from two different TiDB Cloud projects? {#how-to-connect-to-the-same-kafka-private-link-service-from-two-different-tidb-cloud-projects}
