...
# What's New in TiDB 5.0 {#what-s-new-in-tidb-5-0}
...
#### List partitioning (<strong>Experimental</strong>) {#list-partitioning-strong-experimental-strong}
...
#### List COLUMNS partitioning (<strong>Experimental</strong>) {#list-columns-partitioning-strong-experimental-strong}
...
#### <code>EXCEPT</code> and <code>INTERSECT</code> operators {#code-except-code-and-code-intersect-code-operators}
...
### Enable the Coprocessor cache feature by default {#enable-the-coprocessor-cache-feature-by-default}
...
### Improve the execution performance of <code>delete from table where id &#x3C;? Limit ?</code> statement {#improve-the-execution-performance-of-code-delete-from-table-where-id-x3c-limit-code-statement}
...
### Optimize load base split strategy to solve the performance problem that data cannot be split in some small table hotspot read scenarios {#optimize-load-base-split-strategy-to-solve-the-performance-problem-that-data-cannot-be-split-in-some-small-table-hotspot-read-scenarios}

[#18005](https://github.com/pingcap/tidb/issues/18005)

## Improve stability {#improve-stability}

### Optimize the performance jitter issue caused by imperfect scheduling {#optimize-the-performance-jitter-issue-caused-by-imperfect-scheduling}
...
#### Introduce new scheduling calculation formulas to reduce unnecessary scheduling and performance jitter {#introduce-new-scheduling-calculation-formulas-to-reduce-unnecessary-scheduling-and-performance-jitter}
...
#### Enable the cross-table Region merge feature by default {#enable-the-cross-table-region-merge-feature-by-default}
...
#### Enable the system to automatically adjust the data compaction speed by default to balance the contention for I/O resources between background tasks and foreground reads and writes {#enable-the-system-to-automatically-adjust-the-data-compaction-speed-by-default-to-balance-the-contention-for-i-o-resources-between-background-tasks-and-foreground-reads-and-writes}
...
#### Enable the GC Compaction Filter feature by default to reduce GC's consumption of CPU and I/O resources {#enable-the-gc-compaction-filter-feature-by-default-to-reduce-gc-s-consumption-of-cpu-and-i-o-resources}
...
#### TiFlash limits the compression and data sorting's use of I/O resources (<strong>experimental feature</strong>) {#tiflash-limits-the-compression-and-data-sorting-s-use-of-i-o-resources-strong-experimental-feature-strong}
...
#### Improve the performance of checking scheduling constraints and the performance of fixing the unhealthy Regions in a large cluster {#improve-the-performance-of-checking-scheduling-constraints-and-the-performance-of-fixing-the-unhealthy-regions-in-a-large-cluster}

### Ensure that the execution plans are unchanged as much as possible to avoid performance jitter {#ensure-that-the-execution-plans-are-unchanged-as-much-as-possible-to-avoid-performance-jitter}

[User document](/sql-plan-management.md)

#### SQL Binding supports the <code>INSERT</code>, <code>REPLACE</code>, <code>UPDATE</code>, <code>DELETE</code> statements {#sql-binding-supports-the-code-insert-code-code-replace-code-code-update-code-code-delete-code-statements}
...
#### Automatically capture and bind execution plans {#automatically-capture-and-bind-execution-plans}
...
### Improve stability of TiFlash queries {#improve-stability-of-tiflash-queries}
...
### Improve TiCDC stability and alleviate the OOM issue caused by replicating too much incremental data {#improve-ticdc-stability-and-alleviate-the-oom-issue-caused-by-replicating-too-much-incremental-data}
...
### Improve system availability during Region membership change {#improve-system-availability-during-region-membership-change}
...
### Improve the system availability during network partition {#improve-the-system-availability-during-network-partition}

## Data migration {#data-migration}

### Migrate data from S3/Aurora to TiDB {#migrate-data-from-s3-aurora-to-tidb}
...
### Optimize the data import performance of TiDB Cloud {#optimize-the-data-import-performance-of-tidb-cloud}
...
### Integrate TiDB to Kafka Connect (Confluent Platform) using TiCDC (<strong>experimental feature</strong>) {#integrate-tidb-to-kafka-connect-confluent-platform-using-ticdc-strong-experimental-feature-strong}
...
### Optimize the logic of cluster deployment operations, to help DBAs deploy a set of standard TiDB production cluster faster {#optimize-the-logic-of-cluster-deployment-operations-to-help-dbas-deploy-a-set-of-standard-tidb-production-cluster-faster}
...
### Improve upgrade stability {#improve-upgrade-stability}
...
### Optimize the upgrade time {#optimize-the-upgrade-time}
...
### Support the breakpoint feature {#support-the-breakpoint-feature}
...
### Enhance the functionalities of maintenance and operations {#enhance-the-functionalities-of-maintenance-and-operations}
